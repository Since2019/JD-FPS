...
def finish():
    open(os.path.join(T,'realism_'+name+'_test.json'),'w',encoding='utf-8').write(json.dumps(report,ensure_ascii=False,indent=2));u.unregister_slate_post_tick_callback(handle);L.editor_request_end_play();u.EditorPythonScripting.set_keep_python_script_alive(False)
def shot(label):
    global capture
    if capture is None:capture=u.AutomationLibrary.take_high_res_screenshot(1280,720,os.path.join(T,'realism_'+name+'_'+label+'.png'))
