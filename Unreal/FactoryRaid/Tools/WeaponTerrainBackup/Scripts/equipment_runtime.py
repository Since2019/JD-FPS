...
class Equipment:
    def __init__(self,loot):
        ...
        self.slots['primary2']=item('MK18 风格步枪');self.slots['primary2']['ammo']=30
    ...
    def sync(self):
        ...
        if self.loaded is not entry:
            ...
            if self.loaded:self.loaded['ammo']=s.prop(s.weapon,'Current Bullets',0)
            self.loaded=entry;ammo=entry.setdefault('ammo',30) if entry else 0
