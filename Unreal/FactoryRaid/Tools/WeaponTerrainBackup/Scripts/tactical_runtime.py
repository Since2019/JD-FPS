...
class TacticalSession:
    ...
    def __init__(self,world,player,test=False):
        ...
        self.input_keys={name:input_key(name) for name in ['C','Z','X','One','Two','Q','E','RightMouseButton','F','Tab','Escape','LeftShift','LeftMouseButton','W','A','S','D']}
    ...
    def draw_hud(self):
        ...
        if rounds is not None:lines.append('弹药 %d  |  1/2 选择武器 · X 切枪 · C 蹲下 · Z 趴下  |  %s'%(rounds,{'stand':'站立','crouch':'蹲姿','prone':'卧姿'}[self.stance.mode]))
        if self.inventory:
            lines.append('—— 背包 6 × 5 ——')
