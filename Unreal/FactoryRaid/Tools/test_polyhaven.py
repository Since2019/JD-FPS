...
s=None;phase=0;elapsed=0;capture=None;start=time.monotonic();report={'passed':False};views=[('before',(-1500,50,98),48,-22),('service',(-1500,50,98),48,-22),('crate',(-1460,-100,98),-62,-28)]
def finish():
    open(os.path.join(T,'polyhaven_test.json'),'w').write(json.dumps(report,indent=2));u.unregister_slate_post_tick_callback(handle);L.editor_request_end_play();u.EditorPythonScripting.set_keep_python_script_alive(False)
def tick(dt):
    ...
    try:
        ...
        if idx<len(views):
            ...
            if elapsed>6:
                if capture is None:capture=u.AutomationLibrary.take_high_res_screenshot(1600,900,os.path.join(T,'polyhaven_'+name+'.png'))
